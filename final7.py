...
class ImageContainer(QLabel):
    """图片容器，用于在滚动区域中显示原图并支持点击选点"""
    point_clicked = pyqtSignal(object)  # 发射鼠标事件
    ...
    def display_image(self):
        ...
        if self.origin_img is not None:
            # 转换为RGB
            frame_rgb = cv2.cvtColor(self.origin_img, cv2.COLOR_BGR2RGB)
...
class LargeImageSelectDialog(QDialog):
    """超大窗口精确选择角点对话框"""
    points_selected = pyqtSignal(list)  # 发射信号：四个角点坐标 [(x1,y1), (x2,y2), (x3,y3), (x4,y4)]

    def __init__(self, cv_img, parent=None):
        super().__init__(parent)
        self.origin_img = cv_img
        self.selected_points = []  # 存储四个角点
        self.point_order = 0        # 当前选点序号：0左上 1右上 2右下 3左下
        self.point_labels = ['左上角', '右上角', '右下角', '左下角']
        self.initUI()

    def initUI(self):
        self.setWindowTitle('精确选择迷宫四个角点')
        self.setModal(True)
        ...
        # 顶部提示信息
        self.lbl_info = QLabel(f'请按顺序点击选择四个角点：{self.point_labels[0]}')
        self.lbl_info.setStyleSheet('font-size: 16px; font-weight: bold; color: #2c5aa0; padding: 10px;')
        ...
        self.scroll_area.setStyleSheet('border: 2px solid rgba(44, 90, 160, 0.5); border-radius: 8px;')
        layout.addWidget(self.scroll_area, 1)

        # 创建图片容器
        self.image_container = ImageContainer(self.origin_img)
        self.image_container.point_clicked.connect(self.on_image_click)
        ...
        ''')
        self.btn_confirm.clicked.connect(self.confirm_points)
    ...
    def on_image_click(self, event):
        ...
        # 限制坐标在图片范围内
        click_x = max(0, min(img_w - 1, click_x))
        click_y = max(0, min(img_h - 1, click_y))

        # 保存选点
        self.selected_points.append((click_x, click_y))

        # 更新图片容器显示
        self.image_container.set_selected_points(self.selected_points)

        # 更新提示信息
        if self.point_order < 3:
            self.point_order += 1
            self.lbl_info.setText(f'请按顺序点击选择四个角点：{self.point_labels[self.point_order]}')
        else:
            self.lbl_info.setText('四个角点已选择完毕，点击"确认"完成')

    def clear_points(self):
        """清除所有选点"""
        self.selected_points.clear()
        self.point_order = 0
        self.lbl_info.setText(f'请按顺序点击选择四个角点：{self.point_labels[0]}')
        self.image_container.set_selected_points(self.selected_points)
    ...
    def confirm_points(self):
        ...
        if len(self.selected_points) == 4:
            ...
            QMessageBox.warning(self, '提示', f'请选择完整的4个角点（当前已选择{len(self.selected_points)}个）')
...
class FinalApp(QWidget):
    ...
    def capture_image(self):
        if self.cap and self.cap.isOpened():
            ...
            if ret:
                ...
                # 打开超大窗口选择角点
                self.open_corner_selection_dialog()
    ...
    def open_corner_selection_dialog(self):
        """打开超大窗口选择角点"""
        ...
        # 创建并显示超大窗口对话框
        dialog = LargeImageSelectDialog(self.detected_image, self)
        dialog.points_selected.connect(self.on_corner_points_selected)
    ...
    def on_corner_points_selected(self, points):
        """角点选择完成后的回调"""
        # points格式：[(x1,y1), (x2,y2), (x3,y3), (x4,y4)]
        # 对应：左上、右上、右下、左下
        if len(points) == 4:
            # 更新SpinBox的值
            self.spin_tl_x.setValue(points[0][0])
            self.spin_tl_y.setValue(points[0][1])
            self.spin_tr_x.setValue(points[1][0])
            self.spin_tr_y.setValue(points[1][1])
            self.spin_br_x.setValue(points[2][0])
            self.spin_br_y.setValue(points[2][1])
            self.spin_bl_x.setValue(points[3][0])
            self.spin_bl_y.setValue(points[3][1])

            QMessageBox.information(self, '成功', '角点选择完成！')
    ...
    def plan_path(self):
        ...
        # 如果起点就是终点(0,0)，直接返回空路径
        if self.start_point == (0, 0):
            self.path = []
            self.commands = []
            self.list_path.clear()
            QMessageBox.information(self, '成功', '起点和终点相同，无需移动!')
            self.btn_send.setEnabled(False)
            return
    ...
    def plan_path_old_mode(self):
        ...
        while queue:
            ...
            if not direction or number is None or number <= 0:
                continue
    ...
    def plan_path_new_mode(self):
        ...
        while queue:
            ...
            if direction not in direction_to_heading:
                continue
                
            target_heading = direction_to_heading[direction]
            dr, dc = heading_deltas[target_heading]
            new_row = row + dr * number
            new_col = col + dc * number
            ...
            turn_diff = (target_heading - heading) % 4
            
            if turn_diff == 0:
                cmd = f'FORWARD {number}'
                new_heading = heading
            elif turn_diff == 1:
                cmd = f'RIGHT {number}'
                new_heading = target_heading
            elif turn_diff == 2:
                cmd = f'BACKWARD {number}'
                new_heading = heading
            elif turn_diff == 3:
                cmd = f'LEFT {number}'
                new_heading = target_heading
            
            if not visited[new_row][new_col][new_heading]:
                visited[new_row][new_col][new_heading] = True
                
                new_path = path.copy()
                new_path.append((new_row, new_col, cmd))
    ...
    def generate_return_path(self):
        ...
        if use_new_mode:
            # 获取完整的路径点（包括起点）
            full_path = [(self.start_point[0], self.start_point[1], '')] + self.path
            
            # 计算到达终点时的车头方向
            current_h = 0  # 初始车头方向向上 (FORWARD)
            for p in self.path:
                cmd = p[2]
                direction = cmd.split()[0]
                if direction == 'RIGHT':
                    current_h = (current_h + 1) % 4
                elif direction == 'LEFT':
                    current_h = (current_h - 1) % 4
                # FORWARD 和 BACKWARD 车头方向不变
            
            # 逆向遍历路径，从终点回到起点
            for i in range(len(full_path)-1, 0, -1):
                # 获取当前位置和目标位置（上一个位置）
                from_row, from_col = full_path[i][0], full_path[i][1]
                to_row, to_col = full_path[i-1][0], full_path[i-1][1]
                ...
                if dr < 0:
                    target_dir = 0  # 需要向上移动 (FORWARD)
                elif dr > 0:
                    target_dir = 2  # 需要向下移动 (BACKWARD)
                elif dc > 0:
                    target_dir = 1  # 需要向右移动 (RIGHT)
                else:
                    target_dir = 3  # 需要向左移动 (LEFT)
                
                # 计算需要的转向命令
                # 当前车头是current_h，需要转到target_dir方向
                turn_diff = (target_dir - current_h) % 4
                ...
                # 计算移动格数（保持与去程相同）
                number = max(abs(dr), abs(dc))
                
                return_commands.append(f'{cmd_dir} {number}')
